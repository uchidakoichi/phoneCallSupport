...
- **2026-06-13 の3変更（印刷刷新・音読練習の全文表示・マジックリンク認証）は実機ブラウザでの目視確認が未実施**。特にマジックリンクは「メール受信→リンクタップ→自動ログイン」の一連の流れをデプロイ後に実際に試す必要あり
...
- **2026-06-14 のふじキュン演出はローカル（静的サーバ）で目視確認済み**だが、API を伴う一連の流れ（考え中ポーズ→台本完成→コピー→報告カード→ハート）は本番デプロイ後に通し確認が必要

### 次にやること
- クラウド同期（マジックリンクログイン → 履歴引き継ぎ）の動作確認
- 本番での「考え中→完成→コピー→報告カード」のポーズ遷移の通し確認
