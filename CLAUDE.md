...
- ~~iOS Safari での WAV 変換は要確認~~ → **iPhone（iOS Safari）で音読練習の動作確認済み（2026-06-16）**。`decodeAudioData`/`OfflineAudioContext` での WAV 変換も問題なし
- **残る軽微な制約**：90秒超の録音は 4.5MB（Vercel ボディ上限）対策で先頭90秒のみ分析される（通常の練習読みでは問題なし）。長尺対応が必要になったら 16kHz より低いサンプルレートか分割送信を検討
